...
export default function CustomDropdown({ label, options, selectedOption, onSelect }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <label className="block text-sm font-semibold leading-6 text-neutral-700 mb-1">{label}</label>
      <Listbox value={selectedOption} onChange={handleOptionClick}>
        {({ open }) => (
          <>
            <div className="relative mt-2">
              <Listbox.Button className="relative w-full cursor-pointer rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-sm text-neutral-700 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-1 focus:ring-c-teal hover:ring-1 hover:ring-c-teal">
                      ...
                      {({ selected, active }) => (
                        <>
                          <div className="flex items-center">
                            <span className={`ml-1 block truncate ${selected ? 'font-semibold' : 'font-normal'}`}>
                              {option}
                            </span>
                          </div>
